    ...
    async def rank_search_helper(self, ctx, modes, name):
        ...
        async with self.lock:
            ...
            if found_name:
                ...
                embed.set_footer(text=f'T: {end_time:.1f}s | Levels: #{total_level_rank if total_level_rank else "NA"} - {total_levels:,} | XP: #{total_xp_rank if total_xp_rank else "NA"} - {total_exp:,}')
